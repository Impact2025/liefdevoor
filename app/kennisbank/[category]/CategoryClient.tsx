...
export default function CategoryClient({
  ...
  const [subcategories] = useState<Category[]>(initialSubcategories)
  ...
  // Only refetch when filters actually change (skip initial render - data comes from server)
  const [hasInteracted, setHasInteracted] = useState(false)
  useEffect(() => {
    if (hasInteracted) {
      fetchArticles()
    }
  }, [fetchArticles, hasInteracted])
          ...
          <div className="flex-1 min-w-0">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-1 mt-8">
                <button
                  onClick={() => { setCurrentPage(p => Math.max(1, p - 1)); setHasInteracted(true) }}
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      key={page}
                      onClick={() => { setCurrentPage(page); setHasInteracted(true) }}
                ...
                <button
                  onClick={() => { setCurrentPage(p => Math.min(totalPages, p + 1)); setHasInteracted(true) }}
