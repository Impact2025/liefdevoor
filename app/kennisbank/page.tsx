...
async function getKennisbankData() {
  const [categories, featuredArticles, totalArticles] = await Promise.all([
    prisma.knowledgeBaseCategory.findMany({
      where: { isVisible: true, isProfessionalOnly: false, parentId: null },
      select: {
        id: true,
        name: true,
        nameNl: true,
  ...
  const categoriesWithCount = await Promise.all(
    categories.map(async (cat) => ({
      ...cat,
      articleCount: await prisma.knowledgeBaseArticle.count({
        where: { isPublished: true, category: { slug: cat.slug } },
      }),
    }))
  )

  return { categories: categoriesWithCount, featuredArticles, totalArticles }
}

export default async function KennisbankPage() {
  const { categories, featuredArticles, totalArticles } = await getKennisbankData()
      ...
      <section className="border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <form action="/kennisbank/zoeken" method="get" className="mt-8">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="mt-1 text-sm text-gray-500 line-clamp-2">
                        {category.descriptionNl || category.description || 'Bekijk alle artikelen'}
                      </p>
                      <span className="mt-2 inline-block text-xs text-gray-400">
                        {articleCount} artikel{articleCount !== 1 ? 'en' : ''}
                      </span>
                    </div>
                ...
                {featuredArticles.map((article) => (
                  ...
                  >
                    ...
                    <div className="mt-1.5 flex items-center gap-2 text-xs text-gray-400">
                      <span>{article.category?.nameNl || article.category?.name}</span>
                      {article.hasEasyRead && (
                        <>
                          <span>•</span>
