...
import { type DoelgroepData } from '@/lib/doelgroepen-data'
...
/**
 * De doelgroepdata komt als prop binnen vanuit de server component, niet meer
 * uit useParams + useEffect. Daarvoor rende de hele pagina client-side: de
 * server-HTML bevatte alleen een laadspinner, zonder h1, koppen of tekst. De
 * pagina's bleven daardoor op positie 20-50 hangen terwijl de wél
 * server-gerenderde /veilig-daten-lvb goed scoort.
 *
 * Blogs en kennisbankartikelen blijven client-side nagesleept; dat is
 * aanvullende content die niet in de eerste crawl hoeft te staan.
 */
export default function DoelgroepLandingClient({ data }: { data: DoelgroepData }) {
  ...
  useEffect(() => {
    ...
    // data is stabiel per pagina; slug volstaat als dependency
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 w-full py-32">
          ...
          >
            ...
            {/* Main Headline - zoekwoordgericht, met de emotionele heroTitle
                als zichtbare subkop eronder (zie seoH1 in doelgroepen-data) */}
            <h1 className={`${headingSize} font-extrabold tracking-tight leading-[1.1] mb-6`}>
              {data.seoH1 || data.heroTitle}
            </h1>

            {data.seoH1 && (
              <p className={`${textSize} md:text-3xl font-semibold text-white/95 mb-6 leading-snug max-w-2xl`}>
                {data.heroTitle}
              </p>
            )}
